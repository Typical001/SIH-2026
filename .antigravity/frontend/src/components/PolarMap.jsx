...
function Fit({points}) {
 ...
 useEffect(()=>{
  ...
  const observer=new ResizeObserver(()=>{
   ...
   // Stop zoom-out at a single complete world sized for the current viewport.
   map.setMinZoom(Math.min(0,map.getBoundsZoom(WORLD,false,L.point(24,24))));
  });
...
function Click({mode,onClick}) {useMapEvents({click:e=>{if(mode==='MID_OCEAN_COORDINATES')onClick?.([e.latlng.lat,e.latlng.lng]);}});return null;}
function VectorLayer({data,label,color}) {
 ...
 return (data?.features||[]).filter(f=>bounds.contains([f.geometry.coordinates[1],f.geometry.coordinates[0]])).slice(0,100).map((f,i)=>{
 const [lon,lat]=f.geometry.coordinates,p=f.properties;
 return <CircleMarker key={i} center={[lat,lon]} radius={3} renderer={renderer} pathOptions={{color}}><Popup><strong>{label}</strong><p>{p.speed_knots ?? '—'} kn eastward</p><p>{p.source}</p></Popup></CircleMarker>;
 });
}
export default function PolarMap({waypoints=[],icebergsPresent=[],icebergsPredicted=[],forecastHours=72,safetyBufferKm=25,layerVisibility={},byuIcebergData,sarFootprintsData,seaIceLayerData,oceanCurrentsData,weatherWindData,origin,destination,originLabel,destLabel,departureMode,onMapClickCoord,paretoRoutes,activeRouteType='BALANCED',onSelectRouteType}) {
 ...
 useEffect(()=>()=>routeRenderer.remove(),[routeRenderer]);
 ...
 <MapContainer center={[-55,45]} zoom={3} minZoom={-2} zoomSnap={0.1} zoomDelta={0.5} maxZoom={19} worldCopyJump preferCanvas className="w-full h-full" style={{background:'#aad3df'}}>
  <Pane name="local-coastline" style={{zIndex:150}}>
   {base&&<GeoJSON pane="local-coastline" data={base} interactive={false} style={f=>({color:f.properties.kind==='shelf'?'#ffffff':'#bac9bc',weight:1,fillColor:f.properties.kind==='shelf'?'#e0f5fa':'#718678',fillOpacity:1})}/>}
  </Pane>
  ...
  {layerVisibility.seaIce&&seaIceLayerData&&<GeoJSON key="sea-ice" data={seaIceLayerData} style={f=>({color:'#38bdf8',weight:.2,fillOpacity:f.properties.sea_ice_concentration*.4})} onEachFeature={(f,l)=>l.bindTooltip('Calculated sea ice: '+f.properties.sea_ice_percent+'% (latitude proxy, not observed)')}/>}
  {layerVisibility.bathymetry&&seaIceLayerData&&<GeoJSON key="depth-estimate" data={seaIceLayerData} style={f=>({color:'#818cf8',weight:.4,fillOpacity:.07+f.properties.sea_ice_concentration*.12})} onEachFeature={(f,l)=>l.bindTooltip('Illustrative depth estimate: '+Math.round(200+4000*(1-f.properties.sea_ice_concentration)**2)+' m. No measured bathymetry; not used for clearance.')}/>}
  {layerVisibility.sarCandidates&&sarFootprintsData&&<GeoJSON key="inspection" data={sarFootprintsData} style={{color:'#06b6d4',weight:1,fillOpacity:.1}} onEachFeature={(f,l)=>l.bindTooltip(f.properties.name+': calculated inspection box; no SAR acquisition supplied')}/>}
  {layerVisibility.refIcebergs&&(byuIcebergData?.features||[]).map(f=><CircleMarker key={f.properties.id} center={[f.geometry.coordinates[1],f.geometry.coordinates[0]]} radius={3} pathOptions={{color:'#c084fc',weight:1}}><Popup><strong>{f.properties.id} · BYU reference</strong><p>{f.properties.observation_date}</p><p>Earlier observation; not an additional current obstacle.</p></Popup></CircleMarker>)}
  ...
  {layerVisibility.optimizedRoutes&&[...(paretoRoutes?.features||[])].sort((a,b)=>Number(a.properties.route_type===activeRouteType)-Number(b.properties.route_type===activeRouteType)).map(f=><Polyline renderer={routeRenderer} pane="route-profiles" bubblingMouseEvents={false} key={f.properties.route_type} positions={f.properties.waypoints_latlon} pathOptions={{color:f.properties.color,weight:f.properties.route_type===activeRouteType?5:3,opacity:f.properties.route_type===activeRouteType?1:.8,dashArray:f.properties.route_type===activeRouteType?undefined:(f.properties.route_type==='FASTEST'?'3 6':'10 6')}} eventHandlers={{click:()=>onSelectRouteType?.(f.properties.route_type)}}><Popup><strong>{f.properties.route_type}</strong><p>{f.properties.objective}</p><p>{f.properties.eta_hours} h · {f.properties.ice_exposure_hours} ice-exposure hours</p><p>{f.properties.shared_corridor_with?.length?`Shares corridor with ${f.properties.shared_corridor_with.join(', ')}`:'Geometry checked against bundled obstacles.'}</p>{f.properties.feasibility_status}</Popup></Polyline>)}
  </Pane>
  {mapOrigin&&<CircleMarker center={[mapOrigin.lat,mapOrigin.lon]} radius={6} pathOptions={{color:'#22d3ee'}}><Popup>{originLabel} · Offshore planning start</Popup></CircleMarker>}
  {mapEnd&&<CircleMarker center={[mapEnd.lat,mapEnd.lon]} radius={6} pathOptions={{color:'#34d399'}}><Popup>{destLabel} · Offshore planning approach</Popup></CircleMarker>}
